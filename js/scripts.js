$(function () {
	...
	// Поиск
	$('#search_modal form .input').keydown(function () {
		let _self = $(this)

		setTimeout(() => {
			_self.val().length
				? $('#search_modal form .in_category .categories input').prop('disabled', false)
				: $('#search_modal form .in_category .categories input').prop('disabled', true)
		})
	})
